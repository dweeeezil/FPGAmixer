...
7. When the first multichannel front door (USB or AVB) works: repeat the power-cycle restore test with real multichannel audio on every crosspoint (the Pmod bench only exercises the two left channels).
