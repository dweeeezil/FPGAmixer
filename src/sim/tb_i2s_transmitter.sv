...
module tb_i2s_transmitter;
    ...
    // `captured`. MUST be called immediately after that channel's LRCK edge:
    // it skips one rising edge (the delay bit) then captures 24. It does NOT
    // re-align itself, so each channel is preceded by an explicit LRCK-edge
    // wait at the call site (negedge for left, posedge for right). The earlier
    // version captured L and R back-to-back without re-aligning, so R started
    // 7 edges into the wrong place and was always wrong.
    ...
    initial begin
        ...
        @(posedge lrck);              // re-align to the L->R boundary before R
