...
module tb_i2s_receiver;
    ...
    // ------- Stimulus helper -------
    // Drives a 24-bit word onto sdata_i over one LRCK half-period, matching the
    // timing of the real i2s_transmitter (which is hardware-validated):
    //   - data changes on falling SCLK
    //   - the MSB is driven on the FIRST falling SCLK after the LRCK edge
    //     (the receiver skips its own first rising edge as the "delay bit", so
    //     the transmitter does NOT insert an extra delay bit of its own -- the
    //     earlier version did, which shifted every captured word right by one)
    //   - then MSB..LSB, then zeros until LRCK toggles
    //
    // The trailing-zero loop runs until the LRCK toggles, so this task is
    // self-aligning: it always returns exactly at the next LRCK edge, and the
    // following call therefore starts correctly phased for the next channel.
    //
    ...
    task automatic drive_word(input logic [23:0] word);
        ...
        // 24 bits, MSB first. MSB on the first falling SCLK after the LRCK edge.
        for (int i = 23; i >= 0; i--) begin
            @(negedge sclk); sdata_i = word[i];
        end

        // Trailing zeros until LRCK toggles (re-aligns to the channel boundary).
